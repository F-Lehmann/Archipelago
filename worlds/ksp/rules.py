...
if TYPE_CHECKING:
    from . import KerbalSpaceProgramWorld
...
# swim speeds: https://KerbalSpaceProgram.fandom.com/wiki/Swimming_Speed
...
def set_rules(KerbalSpaceProgram_world: "KerbalSpaceProgramWorld"):
    player = KerbalSpaceProgram_world.player
    multiworld = KerbalSpaceProgram_world.multiworld

    for loc in location_table.values():
        set_location_rule(multiworld, player, loc)

    if KerbalSpaceProgram_world.creatures_to_scan:
        option = multiworld.creature_scan_logic[player]

        for creature_name in KerbalSpaceProgram_world.creatures_to_scan:
